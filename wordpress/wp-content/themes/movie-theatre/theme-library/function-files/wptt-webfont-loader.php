...
if ( ! class_exists( 'WPTT_WebFont_Loader' ) ) {
	...
	class WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.1.0
		 * @param string $movie_theatre_url The remote URL.
		 */
		public function __construct( $movie_theatre_url = '' ) {
			$this->remote_url = $movie_theatre_url;
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $font_family => $files ) {
				...
				foreach ( $files as $movie_theatre_url ) {

					// Get the filename.
					$filename  = basename( wp_parse_url( $movie_theatre_url, PHP_URL_PATH ) );
					...
					$tmp_path = download_url( $movie_theatre_url );
			...
			if ( $change ) {
				...
				foreach ( $stored as $movie_theatre_url => $path ) {
					if ( ! file_exists( $path ) ) {
						unset( $stored[ $movie_theatre_url ] );
					}
				}
		 ...
		 * @access public
		 * @since 1.0.0
		 * @param string $movie_theatre_format The format to be used. Use "woff" or "woff2".
		 * @return void
		 */
		public function set_font_format( $movie_theatre_format = 'woff2' ) {
			$this->font_format = $movie_theatre_format;
		}
		 ...
		 * @access protected
		 *
		 * @param string $movie_theatre_url The URL.
		 *
		 * @return string
		 */
		protected function get_absolute_path( $movie_theatre_url ) {

			// If dealing with a root-relative URL.
			if ( 0 === stripos( $movie_theatre_url, '/' ) ) {
				$movie_theatre_parsed_url = parse_url( $this->remote_url );
				return $movie_theatre_parsed_url['scheme'] . '://' . $movie_theatre_parsed_url['hostname'] . $movie_theatre_url;
			}

			return $movie_theatre_url;
		}
	}
}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	 ...
	 * @since 1.0.0
	 *
	 * @param string $movie_theatre_url    The URL of the remote webfont.
	 * @param string $movie_theatre_format The font-format. If you need to support IE, change this to "woff".
	 *
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_styles( $movie_theatre_url, $movie_theatre_format = 'woff2' ) {
		$movie_theatre_font = new WPTT_WebFont_Loader( $movie_theatre_url );
		$movie_theatre_font->set_font_format( $movie_theatre_format );
		return $movie_theatre_font->get_styles();
	}
}
...
if ( ! function_exists( 'wptt_get_webfont_url' ) ) {
	 ...
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_url( $movie_theatre_url, $movie_theatre_format = 'woff2' ) {
		$movie_theatre_font = new WPTT_WebFont_Loader( $movie_theatre_url );
		$movie_theatre_font->set_font_format( $movie_theatre_format );
		return $movie_theatre_font->get_url();
	}
}